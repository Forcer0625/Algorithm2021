...
{   
    ...
    public int[][] bestPhone(int[][] inputArr)
    {
        ...
        DualPivotQuicksort.sort(inputArr,0,len,null,0,0);
...
final class DualPivotQuicksort {
     ...
     */
    private static boolean lesseq(int[] a,int[] b)
    {
        if(a[0] == b[0])
            return (a[1]<=b[1]);
        return (a[0] < b[0]);      
    ...
    public static boolean greateq(int[] a,int[] b)
    {
        if(a[0]==b[0])
            return a[1]>=b[1];
        return a[0]>b[0];
    ...
    static void sort(int[][] a, int left, int right,
        ...
        for (int k = left; k < right; run[count] = k) {
            ...
            if (less(a[k],a[k+1])){//a[k] < a[k + 1]) { // ascending
                while (++k <= right && lesseq(a[k-1],a[k]));//a[k - 1] <= a[k]);
            } else if (lesseq(a[k+1],a[k])){//a[k] > a[k + 1]) { // descending
                while (++k <= right && less(a[k],a[k-1]));//a[k - 1] >= a[k]);
            ...
            // ascending sequence
            if (run[count] > left && less(a[run[count]-1],a[run[count]]))
            {//a[run[count]] >= a[run[count] - 1]) {
                count--;
            }
        ...
        for (int last; count > 1; count = last) {
            for (int k = (last = 0) + 2; k <= count; k += 2) {
                ...
                for (int i = run[k - 2], p = i, q = mi; i < hi; ++i) {
                    if (q >= hi || p < mi && lesseq(a[p + ao],a[q + ao])){//a[p + ao] <= a[q + ao]) {
                        b[i + bo] = a[p++ + ao];
                    } else {
                        b[i + bo] = a[q++ + ao];
                    }
    ...
    private static void sort(int[][] a, int left, int right, boolean leftmost) {
        ...
        if (length < INSERTION_SORT_THRESHOLD) {
            if (leftmost) {
                ...
                } while (greateq(a[++left],a[left - 1]));// a[++left] >= a[left - 1]);
        ...
        if (a[e1] != a[e2] && a[e2] != a[e3] && a[e3] != a[e4] && a[e4] != a[e5]) {
            ...
            for (int k = less - 1; ++k <= great; ) {
                ...
                if (less(ak , pivot1)) { // Move a[k] to left part
                    ...
                    a[less] = ak;
                    ++less;
                } else if (great(ak , pivot2)) { // Move a[k] to right part
                    while (great(a[great] , pivot2)) {
                        if (great-- == k) {
                            break outer;
                        }
